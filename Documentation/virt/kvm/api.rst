...
:Parameters:    args[0] - boolean value specifying whether to enable or
                disable dirty quota throttling (true and false
                respectively)
...
size time intervals. To enforce this, two variables namely
vcpu_dirty_count and vcpu_dirty_quota are defined for every vCPU which
store the number of pages the vCPU has dirtied since its creation and the
number of pages it is allowed to dirty without an intervention from
userspace, respectively.

When the guest tries to dirty a page, it leads to a vmexit as each page is
write-protected. In the vmexit path, we increment the vcpu_dirty_count for
the corresponding vCPU. Then, we check if the vCPU has exhausted its
quota. If yes, we exit to userspace with a custom exit reason
KVM_EXIT_DIRTY_QUOTA_FULL. This "quota full" event is further handled on
the userspace side.

The kvm flag dirty_quota_throttling is defined to enable/disable dirty
quota throttling, i.e. exhausting the dirty quota can lead to an exit to
userspace only if this flag is set.
